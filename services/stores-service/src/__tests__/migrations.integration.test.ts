...
runDescribe('Migraciones SQL stores-service (integración Postgres)', () => {
    let pool: any;
    ...
    test('esquema base: tienda y mesa presentes', async () => {
        expect(await tableExists(pool, 'tienda')).toBe(true);
        expect(await tableExists(pool, 'mesa')).toBe(true);
    });

    test('002: columnas de jerarquía agregadas', async () => {
        expect(await columnExists(pool, 'tienda', 'fk_ciudad_id')).toBe(true);
    });
