...
const createMovement = ({ tipo_mov, fecha_mov, cantidad, cod_prod, fk_cod_prov, fk_id_vent, desc_mov, store_id = 1 }) =>
    db.query(
        `INSERT INTO inventario (tipo_mov, fecha_mov, cantidad, cod_prod, fk_cod_prov, fk_id_vent, desc_mov, store_id)
         ...
         RETURNING *`,
        [tipo_mov, fecha_mov || new Date(), cantidad, cod_prod, fk_cod_prov || null, fk_id_vent || null, desc_mov || null, store_id]
    );
 ...
 */
const updateStock = async (cod_prod, delta, fk_cod_prov = null, fecha_vencimiento = null, store_id = 1) => {
    ...
    try {
        ...
        if (delta > 0) {
            ...
            const resLote = await client.query(
                `INSERT INTO lotes (cod_prod, numero_lote, fecha_vencimiento, cantidad_inicial, cantidad_actual, store_id)
                 VALUES ($1, $2, $3, $4, $4, $5) RETURNING *`,
                [cod_prod, numeroLote, fecha_vencimiento, delta, store_id]
            );
