...
runDescribe('Migraciones SQL inventory-service (integración Postgres)', () => {
    let pool: any;
    ...
    test('esquema base: inventario y proveedor presentes', async () => {
        expect(await tableExists(pool, 'inventario')).toBe(true);
        expect(await tableExists(pool, 'proveedor')).toBe(true);
        expect(await tableExists(pool, 'suministra')).toBe(true);
    });

    test('010: tabla lotes_kardex agregada', async () => {
        expect(await tableExists(pool, 'lotes_kardex')).toBe(true);
    });

    test('columnas añadidas en migraciones', async () => {
        expect(await columnExists(pool, 'inventario', 'stock_minimo')).toBe(true);
        expect(await columnExists(pool, 'inventario', 'fk_cod_prov')).toBe(true);
        expect(await columnExists(pool, 'inventario', 'store_id')).toBe(true);
    });
