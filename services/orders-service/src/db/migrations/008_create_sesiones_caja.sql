...
VALUES (1, '2023-01-01 00:00:00', CURRENT_TIMESTAMP, 'CERRADA', (SELECT COALESCE(SUM(montofinal_vent), 0) FROM venta), 1)
...
UPDATE venta SET sesion_id = 1 WHERE sesion_id IS NULL;

-- Opcional: Hacer sesion_id obligatoria para futuras ventas (si queremos forzarlo)
-- ALTER TABLE venta ALTER COLUMN sesion_id SET NOT NULL;

-- Down Migration
ALTER TABLE venta DROP COLUMN IF EXISTS sesion_id;
