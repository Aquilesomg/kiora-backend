 ...
 * @param {{ metodopago_usu?: string, items: Array, store_id?: number, tipo_entrega?: string, fk_id_mesa?: number }} data
 */
export async function createOrder(data: any) {
    const KIOSKO_USER_ID = Number(process.env.KIOSKO_USER_ID || 1);
    const order = await orderRepository.createWithItems({
        ...data,
        id_usu: KIOSKO_USER_ID,
        descuento_global: data.descuento_global,
        store_id: data.store_id || 1,
        tipo_entrega: data.tipo_entrega || 'PICKUP',
        fk_id_mesa: data.fk_id_mesa || null
    });
    logger.info('Venta creada', { id_vent: order.id_vent, store_id: order.store_id });
    return order;
}
 ...
 */
export async function completeOrder(orderId: string | number, reqHeaders: any, stripePaymentId?: string) {
    ...
    try {
        ...
        const result = await orderRepository.updateStatus(orderId, 'completada', client as any);
        ...
        // 2. Generar factura
        const totalQty = order.items.reduce((sum: number, i: any) => sum + i.cantidad, 0);
        ...
        return { ok: false, error: `Error en transacción: ${(err as Error).message}`, code: 'TRANSACTION_ERROR', status: 500 };
    } finally {
        client.release();
    }
    ...
    try {
        ...
        logger.warn('No se pudo notificar nueva venta al Dashboard (WebSocket)', { error: (wsErr as Error).message });
    }
 ...
 */
export async function updateStatus(orderId: string | number, estado: string, reqHeaders: any) {
    ...
    if (estado === 'reembolsada') {
        ...
        if (order.stripe_payment_id) {
            try {
                ...
                logger.error('CRÍTICO: Falló reembolso Stripe', {
                    orderId, stripe_payment_id: order.stripe_payment_id, error: (stripeErr as Error).message,
                });
        ...
        try {
            ...
            const updated = await orderRepository.updateStatus(orderId, 'reembolsada', client as any);
            ...
            await client.query('ROLLBACK');
            logger.error('Error en transacción de reembolso Outbox', { orderId, error: (err as Error).message });
            throw err;
        } finally {
            client.release();
        }
