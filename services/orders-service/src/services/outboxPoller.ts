...
export async function processEvent(event: any) {
    ...
    case 'inventory.movement': {
        ...
        try {
            ...
            if ((err as any).status && (err as any).status >= 400 && (err as any).status < 500) {
                logger.warn('Outbox: error de negocio en inventory', {
                    id, event_type, status: (err as any).status,
                    cod_prod: payload.cod_prod, error: (err as Error).message,
                });
            ...
            logger.warn('Outbox: fallo al procesar evento', {
                id, event_type, error: (err as Error).message, retry_count: event.retry_count,
            });
        ...
        try {
            ...
            if ((err as any).status && (err as any).status === 422) {
                logger.error('Outbox factus.invoice: error de validación Factus', {
                    id, orderId: payload.orderId, errors: (err as any).factusErrors,
                });
        ...
        try {
            ...
            if ((err as any).status && (err as any).status === 422) {
                logger.error('Outbox factus.credit_note: error de validación Factus', {
                    id, errors: (err as any).factusErrors,
                });
 ...
 */
export async function compensateOrder(orderId: string | number, reason: string) {
