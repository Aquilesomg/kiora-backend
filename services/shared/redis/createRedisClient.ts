...
export interface RedisClientOptions {
  /** Nombre de la conexión para logs y métricas */
  name?: string;
  /** Modo de conexión: 'pub' para publicar, 'sub' para suscribirse (pub/sub) */
  role?: 'default' | 'pub' | 'sub';
  /** Activar lazy connect (no lanza error en arranque si Redis no está disponible) */
  lazyConnect?: boolean;
  /** Número máximo de reintentos antes de rendirse. null = infinito */
  ...
  if (sentinelHostsRaw) {
    ...
    return new Redis({
      ...
      retryStrategy: (times: number) => {
        if (times > 10) return null;  // Rendirse tras 10 intentos
 ...
 */
export function createRedisAdapterClients(): { pubClient: Redis; subClient: Redis } {
