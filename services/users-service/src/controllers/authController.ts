...
import { addToBlacklist } from '../middleware/authMiddleware';
...
export const login = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    ...
    try {
        ...
        const isWebClient = (req.headers['x-client-type'] as string) === 'web';
        ...
        logActivity({ user_email: correo_usu, user_name: usuario.nom_usu, action: 'login', entity_type: 'user', entity_id: usuario.id_usu, details: `Inicio de sesión — ${correo_usu}` });
    } catch (error: unknown) {
        logger.error('Error al iniciar sesión', { error: (error as Error).message });
        next(error);
    }
};

export const refresh = async (req: Request, res: Response, _next: NextFunction): Promise<any> => {
    ...
    try {
        ...
        try {
            ...
            if ((e as any).code === blacklist.BLACKLIST_UNAVAILABLE) {
        ...
        logger.error('Error al verificar Refresh Token', { error: (error as Error).message });
...
export const unlockUser = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<any> => {
    const { id } = req.params;

    try {
        const result = await userRepository.unlock(Number(id));
        ...
        logActivity({ user_email: req.usuario?.correo_usu || '', user_name: req.usuario?.nom_usu || '', action: 'updated', entity_type: 'user', entity_id: id as string, details: `Usuario #${id} desbloqueado por ${req.usuario?.correo_usu}` });
    } catch (error: unknown) {
        logger.error('Error al desbloquear usuario', { error: (error as Error).message });
        next(error);
    }
};
...
export const getMe = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<any> => {
    try {
        ...
        logger.error('Error al obtener perfil', { error: (error as Error).message });
        next(error);
    }
};
...
export const updateUser = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    ...
    try {
        const result = await userRepository.update(Number(id), req.body);
        ...
        logger.error('Error al actualizar usuario', { error: (error as Error).message });
        next(error);
    }
};
...
export const deleteUser = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<any> => {
    ...
    if (parseInt(id as string) === req.usuario!.id_usu) {
        return res.status(403).json({ error: 'No puedes eliminar tu propio usuario.' });
    }

    try {
        const result = await userRepository.softDelete(Number(id));
        ...
        logger.error('Error al eliminar usuario', { error: (error as Error).message });
        next(error);
    }
};

export const updateRole = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<any> => {
    const { id } = req.params;
    ...
    try {
        ...
        logger.error('Error al actualizar rol', { error: (error as Error).message });
        next(error);
    }
};
...
export const forgotPassword = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    ...
    try {
        ...
        try {
            ...
            logger.warn('Redis Streams no disponible, enviando email directamente', { error: (pubErr as Error).message });
...
export const verifyResetCode = async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    ...
    try {
        ...
        logger.error('Error en verify-reset-code', { error: (error as Error).message });
        next(error);
    }
};
...
export const changePassword = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<any> => {
    ...
    try {
        ...
        logger.error('Error en change-password', { error: (error as Error).message });
        next(error);
    }
};

export const adminResetPassword = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<any> => {
    const { id } = req.params;
    const { password } = req.body;

    try {
        const hashedPassword = await bcrypt.hash(password, 10);
        const outcome = await userRepository.updatePassword(Number(id), hashedPassword);
        ...
        logger.error('Error en admin-reset-password', { error: (error as Error).message });
        next(error);
    }
};

export const blockUser = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<any> => {
    const { id } = req.params;
    try {
        const result = await userRepository.blockUser(Number(id), 5);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Usuario no encontrado.' });
        }
        ...
        logActivity({ user_email: req.usuario?.correo_usu || '', user_name: req.usuario?.nom_usu || '', action: 'blocked', entity_type: 'user', entity_id: id as string, details: `Usuario #${id} bloqueado por ${req.usuario?.correo_usu}` });
    } catch (error: unknown) {
        logger.error('Error al bloquear usuario', { error: (error as Error).message });
        next(error);
    }
};

export const getAdminEmails = async (_req: Request, res: Response, next: NextFunction) => {
    try {
        const result = await userRepository.findAdmins();
        const emails = result.rows.map(r => r.correo_usu);
        res.status(200).json({ emails });
    } catch (error: unknown) {
        logger.error('Error al obtener admins', { error: (error as Error).message });
        next(error);
    }
};
